    ...
    __in char* argv[])
{
    ...
    for (int observations = 0; observations < 500; observations++)
    {
        ...
        long elapsed_ns = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

        SmartCache::PushLatencyEventMessage event_msg;
        event_msg.ConfigId = smartCache.m_config.ConfigId;
        event_msg.PushLatencyNS = elapsed_ns;
        smartCache.m_config.SendTelemetryMessage(event_msg);
